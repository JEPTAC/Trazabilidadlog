...
const storageKey = "trazabilidad_logistica_github_pages_fix_v2";
...
async function initFirebase() {
  ...
  try {
    ...
    ]), 6500, "No cargaron los módulos de Firebase");
...
async function bootstrap() {
  try {
    if (!app) throw new Error("No existe el contenedor principal #app");
    ...
    if (state.user && state.mode === "local") {
      await dataLoad();
      render();
      return;
    }
    ...
    console.error(error);
    if (window.__showBootError) window.__showBootError(error.message || error);
    else app.innerHTML = `<div class="loading">Error al iniciar</div>`;
  }
}
